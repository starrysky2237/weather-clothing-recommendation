...
if "city_mode" not in st.session_state:
    st.session_state.city_mode = False

# 텍스트 입력창 클릭 시 콤보박스로 변환
if not st.session_state.city_mode:
    city_input = st.text_input("도시 이름 입력 (예: 서울, 부산, 제주)", key="city_input")
    if city_input == "":
        st.session_state.city_mode = True
        st.experimental_rerun()
else:
    city = st.selectbox("도시 선택", list(city_coordinates.keys()), index=0)
    st.session_state.city_mode = False

# 현재 위치 옵션
use_gps = st.checkbox("📍 현재 위치 자동 감지 (서울 기준)", value=False)
...
with col2:
    if st.button("🌤️ 날씨 확인하기", use_container_width=True):
        if not use_gps and 'city' not in locals():
            city = st.session_state.get("city_input", "서울")
