...
# ===== 입력창 =====
st.markdown("### 🏙️ 도시를 입력하세요 (예: 서울, 부산, 제주)")
if "city" not in st.session_state:
    st.session_state.city = ""

city_input = st.text_input(
    "도시 입력",
    placeholder="도시 이름을 입력하거나 선택하세요",
    key="city_input"
)
...
# 자동완성 드롭다운
filtered = [c for c in city_coordinates.keys() if city_input and city_input in c]
if filtered:
    st.markdown("#### 🔍 자동완성 추천")
    cols = st.columns(3)
...
with col2:
    search = st.button("🌤️ 날씨 확인하기", use_container_width=True)

# ===== 검색 트리거 =====
if search or (city_input and city_input != st.session_state.city):
