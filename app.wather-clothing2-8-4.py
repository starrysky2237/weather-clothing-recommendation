...
city_coordinates = {
    ...
    '안동': (36.5684, 128.7294), '목포': (34.8118, 126.3922), '여수': (34.7604, 127.6622),
    '순천': (34.9506, 127.4872)
}
...
if "city_input" not in st.session_state:
    st.session_state.city_input = "서울"

# 클릭하면 기존 값 삭제되도록 구현
city = st.text_input(
    "도시 이름 입력 (예: 서울, 부산, 제주)",
    value=st.session_state.city_input if option == "🏙️ 도시 직접 입력" else "서울",
    placeholder="도시를 입력 후 Enter ⏎"
)
if city != st.session_state.city_input:
...
with col2:
    # 버튼 클릭 / 자동 / Enter 모두 가능
    if st.button("🌤️ 날씨 확인하기", use_container_width=True) or auto_search or st.session_state.get("city_input"):
        if city.strip():
