...
st.markdown("""
<h1>
🌤️ <span style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%);
...
if st.button("🌤️ 날씨 확인하기"):
    if city not in city_coordinates:
        ...
        st.markdown(f"<h3>{emoji} {weather['weather']}</h3>", unsafe_allow_html=True)
        st.markdown(f"<h3>🌡️ {weather['temp']}°C (체감 {weather['feels_like']}°C)</h3>", unsafe_allow_html=True)
        ...
        st.markdown(f"<h3>👕 오늘의 추천 옷차림</h3><p style='font-size:20px; text-align:center; font-weight:700;'>{clothes}</p>", unsafe_allow_html=True)
