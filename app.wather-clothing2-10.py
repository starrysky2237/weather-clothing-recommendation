...
def get_weather(city):
    ...
    lat, lon = city_coordinates.get(city, (37.5665, 126.9780))  # 기본 서울
...
# ===== 도시 입력/선택 통합 =====
option = st.radio("도시 선택 방식", ["🌆 도시 선택 또는 입력", "📍 현재 위치"], horizontal=True)

if option == "🌆 도시 선택 또는 입력":
    city_input = st.text_input("도시를 입력하거나 선택하세요 (예: 서울, 부산, 제주)", "서울")
    city_select = st.selectbox("주요 도시 목록", ["직접 입력"] + list(city_coordinates.keys()), index=0)
    if city_select != "직접 입력":
        city = city_select
        auto_search = True
    else:
        city = city_input.strip()
        auto_search = bool(city and city != "서울")
else:
    city = "서울"
    auto_search = True
