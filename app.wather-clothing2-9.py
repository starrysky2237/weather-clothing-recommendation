...
# ===== 도시 선택 방식 =====
option = st.radio("도시 선택 방식", ["🏙️ 직접 입력", "🧭 콤보박스 선택", "📍 현재 위치"], horizontal=True)
...
if option == "🏙️ 직접 입력":
    city = st.text_input("도시 이름 입력 (예: 서울, 부산, 제주)", "서울", key="city_input")
    if st.session_state.city_input and st.session_state.city_input != "서울":
        auto_search = True
